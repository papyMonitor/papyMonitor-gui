...
public class widgetSliderH : HBoxContainer
{
    private const int IDX_CMD_JOY_X = 6;
    private const int IDX_GNR_JOY_X = 8;

    // Classes instances
	private MainTop RInst;
    private CheckBox CanEditInst;
    private CheckBox UseJoyPadInst;
    private HSlider HSliderInst;
    ...
    public override void _Ready()
    {
        ...
        HSliderInst = FindNode("HSlider") as HSlider;
        ...
        HSliderInst.Connect("value_changed", this, nameof(_on_HSlider_changed));
    }
	...
	public override void _Process(float delta)
    {
        if (joyEnabled)
        {
            if (BaseIndex == IDX_CMD_JOY_X)
                HSliderInst.Value = (-Input.GetJoyAxis(0, 2) + 1.0f) * 127.5f;

            if (BaseIndex == IDX_GNR_JOY_X)
                HSliderInst.Value = (Input.GetJoyAxis(0, 0) + 1.0f) * 127.5f;         
        }
    }
    ...
    private void _on_HSlider_changed(float value)
    {
